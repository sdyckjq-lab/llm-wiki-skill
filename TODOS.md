...
设计文档选了 Approach B（完整学习驾驶舱），核心骨架落地了，但左侧导航和联动还没做。
以下按优先级排列，A-C 是一组的，建议一起开分支做。

1. **左侧社区导航面板**
   - 新增独立左侧导航区：社区榜（名称、节点数、source 数）+ 推荐起点列表
   - 每个社区取第一个 topic 节点的 label 作为主题提示；第一项默认高亮
   - 约束：需要把 `.app` 两列布局改成三列，或把 `canvas-wrap` 内左侧腾出导航空间

2. **社区榜展示前 3 个社区**
   - 社区过弱时仍按稳定排序选第一社区，但左侧保留前 3 个
   - 避免过度断言"你最该学这个"

3. **点击社区触发左中右联动**
   - 设计文档定义了 6 种触发动作的完整联动表
   - 点击左侧社区 → 中间切到该社区视图 → 右侧更新
   - 点击推荐起点 → 进入路径视图 → drawer 自动展开学习解释
   - 点击图中节点 → 左侧保持 → 中间维持当前模式 → 右侧刷新
