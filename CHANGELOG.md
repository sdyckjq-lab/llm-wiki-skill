...
- 社区阅读默认态关系视觉分层（A 保守对齐第一批，#135）：新建结构跨度选择器（真实边-only 生成森林、按社区规模分档预算、优先串起核心节点与小团块，不再用最高权重边堆叠），替代旧的全量可见边 top-K 截断；Sigma 社区阅读主路径按 communityMapLayer 分支，让结构连接线比背景关系更粗更亮，关系颜色仍只表达关系类型。沿用直线、明亮视觉语言，不引入曲线或粗重主路线。
