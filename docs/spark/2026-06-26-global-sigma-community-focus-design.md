...
但现在点击社区后，地图本身没有形成足够强的“我正在看这个社区”的状态：用户主要靠右侧抽屉理解选择结果，图谱区域更像静态背景。本设计要补的是全局图内部的临时聚焦态。

一句话：

**点击社区后仍停留在全局地图，但地图要轻量聚焦当前社区；真正进入社区仍由抽屉里的“进入社区”按钮负责。**
...
1. 点击社区后，不切到社区视图。
2. 右侧抽屉继续显示社区简介。
3. 全局 Sigma 图进入临时社区聚焦态。
4. 当前社区变清晰、变突出。
5. 其他社区明显弱化但仍可见。
6. 视角只做轻量构图调整，不做强镜头推进。
7. 左上“回全图”按钮复用为退出聚焦入口。
8. 点空白处也可以退出聚焦并清空当前选择。
...
因此，后续实现不应复用 `focusCommunity(id)` 来做全局聚焦。需要新增一个只属于全局 Sigma 的临时状态，例如“全局聚焦社区 id”。它和“进入社区阅读”的 focus 必须分开。
...
- 先关闭工具栏面板，再处理选择或聚焦，保持现有空白点击优先级。

## 聚焦态视觉规则

全局社区聚焦态的核心不是“把当前社区单独放大”，而是“在全局地图上强调当前社区”。
...
点击社区时，选择状态和全局聚焦状态通常同步：

- 选择状态决定抽屉显示什么。
- 全局聚焦状态决定地图如何表达。

退出聚焦时，同时清空选择并关闭抽屉。这样“回全图”和“点空白处”的结果一致，用户不会看到地图已经回全局、抽屉还停在旧社区的割裂状态。
...
全局社区聚焦是新交互能力，应另开 issue 或设计任务实现。它至少会涉及：
...
4. 全局聚焦态点“回全图”：停留在 Sigma，全图恢复普通构图。
5. 全局聚焦态点空白处：退出聚焦，抽屉关闭，选择清空。
6. 连续点击不同社区：抽屉和图谱聚焦同步切换，动画不抖动。
7. 聚焦状态下搜索和筛选：不丢失明确用户状态，不出现空白图。
8. 节点 Pin 和拖拽后的固定位置：不因聚焦和回全图丢失。
9. 右侧抽屉打开时：聚焦社区不被抽屉遮挡。
...
全局 hover 可以在本设计之后另开一份设计：它应基于这里定义的全局聚焦状态继续补，而不是重新定义点击社区的意义。
