...
4. 暂停跟随后，显示“回到底部”入口，提醒用户有最新内容可回到。
5. 用户点击“回到底部”，或自己手动滚回底部附近后，恢复自动跟随。
...
4. 用户点击“回到底部”后，立即回到最新回复，并恢复自动跟随。
5. 用户自己滚回底部后，“回到底部”入口消失，后续回复继续自动跟随。
6. 切换到已有长会话时，默认看到最新消息。
7. 回复出错或停止生成时，界面没有异常跳动。

## 验证方式

推荐验证分三层：

1. 增加前端组件测试，覆盖发送、流式更新、用户上翻、回到底部入口显示与恢复。
