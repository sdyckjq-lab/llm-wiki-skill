...
正式接入需要建立清晰的 Sigma 全局适配层。

适配层负责把现有图谱引擎输出整理成 Sigma/Graphology 能绘制的数据，同时把 Sigma 的点击、悬停和视口事件转回图谱引擎命令。
...
1. 在图谱引擎内建立正式 Sigma 全局渲染适配层。
2. 把现有试验用 Sigma 代码收敛成生产路径，而不是直接复制测试 harness。
3. 让 `createGraphEngine` 或等价入口能根据视角创建全局 Sigma 路径和社区阅读路径。
4. 保持现有 workbench 的 `GraphPanel` 对外能力不大改：数据加载、Pin 持久化、选择回调、打开详情、可见状态通知继续通过当前边界通信。
5. 把 Graphology 和 Sigma 从试验依赖提升为正式运行依赖时，要明确包边界和构建产物影响。
6. 给 WebGL/Sigma 初始化失败增加可测试兜底路径。
7. 把性能 harness 从“试验记录”升级为正式验收门禁。
